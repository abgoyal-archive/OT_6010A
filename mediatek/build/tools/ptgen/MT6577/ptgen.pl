	...
	while($pt_name ne "END"){
		...
		if($EMMC_SUPPORT eq "yes" && $pt_name =~ /MBR/){
			for($tmp_index=1;$tmp_index<=$br_count;$tmp_index++){
				...
				$SIZE_FIELD_KB[$row_t -1] = 32;

				#scream loudly:
				#global only! target jrdsh77_cu_jb is used for diablo, others use different ones
				#diablo has special pro_info offset(16KB bigger that its variants), so set a fake size
				if($PROJECT eq "diablohd") {
...
#****************************************************************************************
# subroutine:  ImagesRework
# input:       @_[0]:  project name
#****************************************************************************************
sub ImagesRework()
{
    my $proj_name = @_[0];
    # get version number
    open(my $fp, "<", "version_$proj_name/version.inc") or die "version_$proj_name/version.inc: $!";
    my @file = <$fp>;
    close($fp);
	...
	my @line = split;
	if (/mbr/i) {
	    my $mbr_ver = substr(@line[2], 3, 3);
	    print "$mbr_ver\n";
	    BinaryCheckSum($mbr_ver, "mediatek/misc/MBR", 0);
	} 
    }
    print "Images rework done!\n";
}


#****************************************************************************************
# subroutine:  BinaryCheckSum
# input:       @_[0]:  version of current build
# input:       @_[1]:  path of target file
# input:       @_[2]:  offset in target file for caculating checksum
#****************************************************************************************
sub BinaryCheckSum()
{
    my $ver = @_[0];
    ...
    read($fp, my $char, 512, $offset);
    my @mbr = unpack("C512", $char);
    ...
    # transform version number to hex
    my @ver_magic = unpack("C*", $ver);
    # ignore verion magic 
    my $ver_rework = (@ver_magic[1]<<8) + @ver_magic[2];
    printf("%#x\n",$ver_rework);
    
    # compute the rework value
    my $add = ($ver_rework & 0x1ffff) - ($sum & 0xffff);
    $add &= 0xffff;
    printf("%#04x\n", $add);
    
    # Write checksum rework value into the binary
    seek($fp, $offset, 0);
    print $fp pack("v1", $add);
